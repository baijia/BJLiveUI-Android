...
class PPTFragment : BasePadFragment(), PPTMenuContract.View {
    ...
    private val presenter by lazy {
        PPTMenuPresenterBridge(this, (activity as LiveRoomTripleActivity).getRouterListener(), routerViewModel)
    }
    private val disposables by lazy {
        CompositeDisposable()
    }
    private var dataBinding: FragmentPadPptBinding? = null
    ...
    override fun observeActions() {
        routerViewModel.actionNavigateToMain.observe(this@PPTFragment, Observer {
            ...
            dataBinding?.run {
                pptviewmodel = this@PPTFragment.pptViewModel
                lifecycleOwner = this@PPTFragment
                pptView?.run {
                    ...
                    try {
                        context?.run {
                            MaterialDialog.Builder(this)
                                    .title(getString(R.string.live_room_ppt_load_error,it))
    ...
    private fun classStart() = with(routerViewModel) {
        if (liveRoom.currentUser.type == LPConstants.LPUserType.Student)
            // 一对一、小班课上课学生自动举手上麦
            if (!liveRoom.isAudition) {
                //试听进入不上台
                if (liveRoom.roomType == LPConstants.LPRoomType.Single || liveRoom.roomType == LPConstants.LPRoomType.SmallGroup) {
                    if (liveRoom.isClassStarted) {
                        ...
                        if (liveRoom.autoOpenCameraStatus) {
                            disposables.add(Observable.timer(500, TimeUnit.MILLISECONDS).observeOn(AndroidSchedulers.mainThread())

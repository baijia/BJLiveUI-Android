...
class OnlineUserFragment : BasePadFragment() {
    ...
    private fun initExpandableListView(){
        ...
        elv_online_group.setOnGroupExpandListener {
            var i = 0
            val count = groupAdapter.getGroupCount()
            while (i < count) {
                if (i != it) {
    ...
    inner class OnlineUserAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun onCreateViewHolder(viewGroup: ViewGroup, position: Int): RecyclerView.ViewHolder {
            return if (getItemViewType(position) == VIEW_TYPE_LOADING) {
                val itemView = LayoutInflater.from(context).inflate(R.layout.item_online_user_loadmore, viewGroup, false)
                LoadingViewHolder(itemView)
            } else {
                val dataBinding: LayoutItemOnlineUserBinding = DataBindingUtil.inflate(LayoutInflater.from(context), R.layout.layout_item_online_user, viewGroup, false)
...
//                    viewHolder.nameTv.text = currentUser.name
    ...
    class OnlineUserViewHolder(val dataBinding: LayoutItemOnlineUserBinding, itemView: View) : RecyclerView.ViewHolder(itemView){
        ...
        val nameTv: TextView = itemView.findViewById(R.id.item_online_user_name)
        val avatarIv: ImageView = itemView.findViewById(R.id.item_online_user_avatar)
    }
}